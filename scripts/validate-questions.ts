...
function validateQuestion(
  ...
  if (!q.text || q.text.trim().length < 20) {
    errors.push({
      code: 'STEM_TOO_SHORT',
      message: `Stem is ${q.text?.length ?? 0} chars, minimum 20`,
    });
  }
  ...
  if (!q.options || q.options.length < 4 || q.options.length > 5) {
    errors.push({
      ...
      message: `${q.options?.length ?? 0} options, must be 4 or 5`,
    });
  }
  ...
  if (
    ...
    q.correctOptionIndex >= (q.options?.length ?? 0)
  ) {
    errors.push({
      code: 'CORRECT_INDEX_OUT_OF_RANGE',
      message: `correctOptionIndex ${q.correctOptionIndex} out of range [0-${(q.options?.length ?? 1) - 1}]`,
    });
  }
    ...
    for (let i = 0; i < q.options.length; i++) {
      if (!q.options[i].text || q.options[i].text.trim().length === 0) {
        errors.push({
          ...
          message: `Option ${q.options[i].label || i} has empty text`,
        });
      }
    }
  }
    ...
    for (const t of optTexts) {
      if (seen.has(t)) {
        errors.push({
          ...
          message: `Duplicate option text: "${t.slice(0, 50)}..."`,
        });
        break;
      }
      seen.add(t);
  ...
  if (!q.explanation?.short || q.explanation.short.trim().length === 0) {
    errors.push({
      code: 'MISSING_EXPLANATION_SHORT',
      message: 'explanation.short is missing or empty',
    });
  } else if (countSentences(q.explanation.short) < 2) {
    errors.push({
      code: 'EXPLANATION_TOO_SHORT',
      message: `explanation.short has ${countSentences(q.explanation.short)} sentence(s), minimum 2`,
    });
  }
  ...
  if (q.options && q.explanation?.whyOthersWrong) {
    for (let i = 0; i < q.options.length; i++) {
      ...
      if (
        !q.explanation.whyOthersWrong[label] ||
        q.explanation.whyOthersWrong[label].trim().length === 0
      ) {
        errors.push({
          code: 'MISSING_WHY_OTHERS_WRONG',
          message: `Missing whyOthersWrong for option ${label}`,
        });
      }
    }
    ...
    errors.push({
      code: 'MISSING_WHY_OTHERS_WRONG',
      message: 'explanation.whyOthersWrong is missing entirely',
    });
  }
      ...
      for (const did of q.domainIds) {
        if (!known.includes(did)) {
          errors.push({
            ...
            message: `Unknown domain ID "${did}" for cert "${certId}"`,
          });
        }
      }
    }
  ...
  if (q.options) {
    for (const opt of q.options) {
      ...
      if (lower.includes('all of the above')) {
        errors.push({
          code: 'ALL_OF_ABOVE_IN_OPTIONS',
          message: `Option ${opt.label} contains "All of the above"`,
        });
      }
      if (lower.includes('none of the above')) {
        errors.push({
          code: 'NONE_OF_ABOVE_IN_OPTIONS',
          message: `Option ${opt.label} contains "None of the above"`,
        });
      }
    }
  }
  ...
  while ((nistMatch = nistPattern.exec(fullText)) !== null) {
    ...
    if (!KNOWN_NIST_SP.includes(spNumber)) {
      errors.push({
        code: 'FABRICATED_STANDARD',
        message: `Potentially fabricated NIST SP ${spNumber} — verify at nist.gov`,
      });
    }
  }
  ...
  if (
    ...
    if (correctLen > maxOtherLen * 1.3) {
      warnings.push({
        ...
        message: 'Correct answer is >30% longer than longest distractor',
      });
    }
  }
  ...
  if (q.options && q.options.length >= 4) {
    ...
    if (maxLen > minLen * 3 && minLen > 0) {
      warnings.push({
        code: 'UNBALANCED_OPTION_LENGTHS',
        message: `Option lengths vary from ${minLen} to ${maxLen} chars (${(maxLen / minLen).toFixed(1)}x ratio)`,
      });
    }
  }
  ...
  if (q.explanation?.short) {
    const hasReference =
      /(?:NIST|ISO|SP\s+\d|CBK|GDPR|HIPAA|SOX|PCI|CSA|OWASP|RFC|Domain\s+\d)/i.test(
        q.explanation.short
      );
    if (!hasReference) {
      warnings.push({
        code: 'NO_REFERENCE_IN_EXPLANATION',
        message: 'explanation.short has no standard/framework reference',
      });
    }
  }
  ...
  for (const term of BIAS_TERMS) {
    ...
    if (stemLower.includes(termLower)) {
      ...
      if (!boldPattern.test(q.text)) {
        warnings.push({
          code: 'POTENTIAL_BIAS',
          message: `Stem contains unbolded bias term: "${term}"`,
        });
  ...
  for (const neg of negativePatterns) {
    if (
      ...
      if (!q.text.includes(`**${neg}**`)) {
        warnings.push({
          code: 'STEM_NEGATIVE_UNBOLD',
          message: `"${neg}" in stem is not bolded`,
        });
      }
    }
  }
  ...
  if (q.tags && q.tags.length > 0 && q.tags.length < 2) {
    warnings.push({
      code: 'LOW_TAG_COUNT',
      message: `Only ${q.tags.length} tag(s), recommend 2+`,
    });
  }
  ...
  if (q.explanation?.whyOthersWrong) {
    for (const [label, text] of Object.entries(q.explanation.whyOthersWrong)) {
      if (text && text.trim().length > 0 && text.trim().length < 30) {
        warnings.push({
          code: 'SHORT_EXPLANATION_DISTRACTOR',
          message: `whyOthersWrong.${label} is very short (${text.trim().length} chars)`,
        });
      }
    }
  }
  ...
  if (q.text && q.text.length < 50 && q.difficulty !== 'easy') {
    warnings.push({
      code: 'STEM_ANSWERABLE_WITHOUT_OPTIONS',
      message:
        'Short stem for non-easy question — may be answerable without reading options',
    });
  }
...
function checkDifficultyDistribution(
  ...
  const TOLERANCE = 0.1; // 10% tolerance per difficulty level
  let passed = true;
  const issues: string[] = [];
  ...
  for (const diff of ['easy', 'medium', 'hard']) {
    ...
    if (delta > TOLERANCE) {
      passed = false;
      issues.push(
        `${diff}: ${(actual[diff] * 100).toFixed(0)}% (target: ${(target[diff] * 100).toFixed(0)}%)`
      );
    }
  }
...
function main(): void {
  ...
  if (!filePath) {
    console.error(
      'Usage: npx tsx scripts/validate-questions.ts <batch-file.json> [--strict]'
    );
    console.error('');
  ...
  const failedResults = results.filter(
    (r) => !r.passed || r.warnings.length > 0
  );
  ...
  console.log(
    `  Easy:   ${counts.easy}/${total} (${total > 0 ? ((counts.easy / total) * 100).toFixed(0) : 0}%)`
  );
  console.log(
    `  Medium: ${counts.medium}/${total} (${total > 0 ? ((counts.medium / total) * 100).toFixed(0) : 0}%)`
  );
  console.log(
    `  Hard:   ${counts.hard}/${total} (${total > 0 ? ((counts.hard / total) * 100).toFixed(0) : 0}%)`
  );
  console.log(
    `  Status: ${diffCheck.passed ? '✅ Within tolerance' : '⚠️  ' + diffCheck.message}`
  );
  console.log('');
