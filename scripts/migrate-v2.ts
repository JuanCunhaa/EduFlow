...
const CERT_TO_STUDY: Record<string, { abbreviation: string; name: string; domains: CertDomain[] }> = {
    CISSP: {
        abbreviation: 'CISSP',
        ...
        domains: [
            ...
            { id: 'd3', abbreviation: 'SA', name: 'Security Architecture and Engineering', order: 2 },
            { id: 'd4', abbreviation: 'CNS', name: 'Communication and Network Security', order: 3 },
            { id: 'd5', abbreviation: 'IAM', name: 'Identity and Access Management', order: 4 },
            { id: 'd6', abbreviation: 'SAT', name: 'Security Assessment and Testing', order: 5 },
            { id: 'd7', abbreviation: 'SO', name: 'Security Operations', order: 6 },
            { id: 'd8', abbreviation: 'SSD', name: 'Software Development Security', order: 7 },
        ],
    },
    CC: {
        abbreviation: 'CC',
        ...
        domains: [
            ...
            { id: 'd2', abbreviation: 'BC', name: 'Business Continuity, Disaster Recovery & Incident Response', order: 1 },
            { id: 'd3', abbreviation: 'AC', name: 'Access Controls Concepts', order: 2 },
    ...
    SSCP: {
        ...
        domains: [
            { id: 'd1', abbreviation: 'SAC', name: 'Security Operations and Administration', order: 0 },
            { id: 'd2', abbreviation: 'AC', name: 'Access Controls', order: 1 },
            { id: 'd3', abbreviation: 'RIF', name: 'Risk Identification, Monitoring, and Analysis', order: 2 },
            { id: 'd4', abbreviation: 'IR', name: 'Incident Response and Recovery', order: 3 },
            { id: 'd5', abbreviation: 'CRY', name: 'Cryptography', order: 4 },
            { id: 'd6', abbreviation: 'NCS', name: 'Network and Communications Security', order: 5 },
            { id: 'd7', abbreviation: 'SSA', name: 'Systems and Application Security', order: 6 },
        ],
    },
    CCSP: {
        abbreviation: 'CCSP',
        ...
        domains: [
            { id: 'd1', abbreviation: 'ARC', name: 'Cloud Concepts, Architecture and Design', order: 0 },
            { id: 'd2', abbreviation: 'DS', name: 'Cloud Data Security', order: 1 },
            { id: 'd3', abbreviation: 'PS', name: 'Cloud Platform and Infrastructure Security', order: 2 },
            { id: 'd4', abbreviation: 'AS', name: 'Cloud Application Security', order: 3 },
            { id: 'd5', abbreviation: 'SO', name: 'Cloud Security Operations', order: 4 },
            { id: 'd6', abbreviation: 'LR', name: 'Legal, Risk and Compliance', order: 5 },
        ],
    },
    CGRC: {
        abbreviation: 'CGRC',
        ...
        domains: [
            { id: 'd1', abbreviation: 'IAP', name: 'Information Security Risk Management Program', order: 0 },
            { id: 'd2', abbreviation: 'SCO', name: 'Scope of the Information System', order: 1 },
            { id: 'd3', abbreviation: 'SEL', name: 'Selection and Approval of Security and Privacy Controls', order: 2 },
            { id: 'd4', abbreviation: 'IMP', name: 'Implementation of Security and Privacy Controls', order: 3 },
            { id: 'd5', abbreviation: 'ASS', name: 'Assessment/Audit of Security and Privacy Controls', order: 4 },
            { id: 'd6', abbreviation: 'AUT', name: 'Authorization/Approval of Information System', order: 5 },
            { id: 'd7', abbreviation: 'CMA', name: 'Continuous Monitoring', order: 6 },
        ],
    },
};
...
async function migrateUser(uid: string): Promise<MigrationStats> {
    ...
    for (const doc of questionsSnap.docs) {
        ...
        if (!cert || !certToStudyId.has(cert)) {
            stats.errors.push(`Question ${doc.id}: no studyId mapping for cert ${cert}`);
            continue;
        }
    ...
    for (const doc of examsSnap.docs) {
        ...
        const cert = data.certification;
        const studyId = cert && certToStudyId.has(cert) ? certToStudyId.get(cert)! : null;
        ...
        if (data.domainScores) {
            for (const [key, value] of Object.entries(data.domainScores)) {
                const ds = value as { domain: string; correct: number; total: number; percentage: number };
                newDomainScores[key] = {
                    ...ds,
                    domainId: key,
                };
    ...
    if (!statsDoc.exists) {
        ...
        const historySnap = await db.collection(`users/${uid}/examHistory`)
    ...
    if (userDoc.exists) {
        ...
        const activeStudyId = targetCert && certToStudyId.has(targetCert)
