...
const STOP_WORDS = new Set([
    ...
    'have', 'will', 'do', 'did', 'does', 'been', 'was', 'were', 'are',
    'be', 'this', 'that', 'these', 'those', 'what', 'when', 'where',
    'how', 'why', 'who', 'whom', 'its', 'it', 'you', 'your', 'they',
    'their', 'them', 'from', 'by', 'as', 'into', 'about', 'than',
    'would', 'should', 'could', 'may', 'might', 'must', 'shall',
    'following', 'most', 'best', 'first', 'primary', 'which',
]);
...
function detectDuplicates(
    ...
    const existingVectors = existingQuestions.map(q => vectorizer.transform(q.text));

    // Pre-compute concept fingerprints for existing questions
    const existingFingerprints = existingQuestions.map(q => conceptFingerprint(q));
    ...
    for (let i = 0; i < newQuestions.length; i++) {
        ...
        for (let j = 0; j < existingQuestions.length; j++) {
            ...
            const effectiveThreshold = conceptMatch ? CONCEPT_OVERLAY : SIMILAR_THRESHOLD;
        ...
        if (bestScore >= DUPLICATE_THRESHOLD) {
            verdict = 'duplicate';
        } else if (bestScore >= SIMILAR_THRESHOLD || (bestConceptMatch && bestScore >= CONCEPT_OVERLAY)) {
            verdict = 'similar';
        } else {
            verdict = 'unique';
        }
        ...
        results.push({
            ...
            existingQuestionId: bestIdx >= 0 ? (existingQuestions[bestIdx].id ?? `existing-${bestIdx}`) : '',
            existingStem: bestIdx >= 0 ? existingQuestions[bestIdx].text.slice(0, 80) : '',
            ...
            if (sim >= SIMILAR_THRESHOLD) {
                ...
                if (sim > existingResult.textSimilarity) {
                    results[j] = {
                        ...
                        conceptMatch: conceptFingerprint(newQuestions[i]) === conceptFingerprint(newQuestions[j]),
...
function main(): void {
    ...
    if (!batchPath) {
        console.error('Usage: npx tsx scripts/duplicate-detector.ts <batch-file.json> --existing <existing-questions.json>');
        console.error('');
        console.error('Options:');
        console.error('  --existing <file>   JSON file with existing questions [{text, domainIds, tags, difficulty, id}]');
        process.exit(2);
    }
    ...
    if (existingPath) {
        ...
        try {
            ...
            console.error(`Error: Cannot read existing questions file "${resolvedExistingPath}"`);
            process.exit(2);
        }
    }
    ...
    if (duplicates.length > 0) {
        ...
        for (const d of duplicates) {
            console.log(`  ❌ Q${d.newQuestionIndex + 1} (score: ${d.textSimilarity})`);
            console.log(`     New:      "${d.newStem}..."`);
            console.log(`     Matches:  "${d.existingStem}..." [${d.existingQuestionId}]`);
            if (d.conceptMatch) console.log(`     ⚙️  Concept fingerprint also matches`);
            console.log('');
        }
    }
    ...
    if (similar.length > 0) {
        ...
        for (const s of similar) {
            console.log(`  ⚠️  Q${s.newQuestionIndex + 1} (score: ${s.textSimilarity})`);
            console.log(`     New:      "${s.newStem}..."`);
            console.log(`     Matches:  "${s.existingStem}..." [${s.existingQuestionId}]`);
            if (s.conceptMatch) console.log(`     ⚙️  Concept fingerprint also matches`);
            console.log('');
        }
    }
    ...
    const reportPath = resolvedBatchPath.replace(/\.json$/, '.duplicate-report.json');
    ...
    if (duplicates.length > 0) {
        ...
        console.log('⚠️  SIMILAR QUESTIONS FOUND — Review recommended but import allowed.');
        process.exit(0);
    } else {
