...
async function migrateUserQuestions(uid: string): Promise<MigrationStats> {
    ...
    for (const doc of snapshot.docs) {
        ...
        if (typeof data.explanation === 'object' && data.explanation !== null && 'short' in data.explanation) {
            stats.skipped++;
            continue;
        }

        // Build new structured explanation
        const shortExplanation: string = typeof data.explanation === 'string' ? data.explanation : '';
...
async function main() {
    ...
    let globalStats: MigrationStats = { total: 0, migrated: 0, skipped: 0, errors: 0 };
    ...
    for (const uid of userIds) {
        try {
            ...
            if (stats.total > 0) {
                console.log(`  ✅ ${uid}: ${stats.migrated} migrated, ${stats.skipped} skipped (${stats.total} total)`);
            }
        } catch (err) {
            globalStats.errors++;
