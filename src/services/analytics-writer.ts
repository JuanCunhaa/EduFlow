...
export function generateAnonId(uid: string): string {
  ...
  return createHmac('sha256', ANALYTICS_HMAC_SECRET).update(uid).digest('hex');
}
...
export async function updateQuestionAggregates(
  ...
  for (const answer of event.answers) {
    ...
    if (snap.exists) {
      await incrementExistingStats(
        docRef,
        snap.data() as AggregatedQuestionStats,
        answer
      );
    } else {
...
async function incrementExistingStats(
  ...
  const newAvgTime = Math.round(
    (existing.avgTimeMs * existing.totalAttempts + answer.timeMs) / newTotal
  );
