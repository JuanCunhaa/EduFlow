...
import type { DripSequenceId, DripEnrollment, DripSequence, DripStep } from '@/types';
...
const SEQUENCES: DripSequence[] = [
    {
        ...
        trigger: 'signup',
        steps: [
            { delayDays: 0, templateId: 'welcome-1', subject: 'Welcome to ExamFlow — Start Your Certification Journey' },
            { delayDays: 2, templateId: 'welcome-2', subject: 'Your personalized study plan is ready' },
            { delayDays: 5, templateId: 'welcome-3', subject: '3 tips to boost your practice exam scores' },
            { delayDays: 10, templateId: 'welcome-4', subject: 'How are you progressing? Check your readiness score' },
        ],
    },
    {
        id: 're_engagement',
        ...
        trigger: 'inactivity',
        steps: [
            { delayDays: 7, templateId: 'reengage-1', subject: "We miss you — here's what you left off on", condition: 'not_active_7d' },
            { delayDays: 14, templateId: 'reengage-2', subject: 'Your certification goal is waiting', condition: 'not_active_7d' },
        ],
    },
    {
        id: 'cert_tips',
        ...
        steps: [
            { delayDays: 0, templateId: 'tips-1', subject: 'Free study resources for your certification' },
            { delayDays: 3, templateId: 'tips-2', subject: '5 mistakes to avoid on exam day' },
            { delayDays: 7, templateId: 'tips-3', subject: 'How to use adaptive practice exams effectively' },
        ],
    },
    {
        id: 'upgrade_nudge',
        ...
        trigger: 'manual',
        steps: [
            { delayDays: 0, templateId: 'upgrade-1', subject: 'Unlock advanced features with ExamFlow Pro', condition: 'free_plan' },
            { delayDays: 5, templateId: 'upgrade-2', subject: 'Your practice results suggest you need more questions', condition: 'free_plan' },
        ],
    },
];
...
export async function enrollInSequence(
    ...
    const existing = await db.collection('drip_enrollments')
        .where('email', '==', email)
...
export async function processDueEnrollments(): Promise<{ processed: number; sent: number }> {
    const db = getAdminDb();
    const now = Date.now();

    const dueSnap = await db.collection('drip_enrollments')
        .where('status', '==', 'active')
    ...
    for (const doc of dueSnap.docs) {
        ...
        if (success) {
            ...
            if (nextStep >= sequence.steps.length) {
                await doc.ref.update({ status: 'completed', completedAt: Date.now(), currentStep: nextStep });
            } else {
...
export async function unsubscribeEmail(email: string): Promise<number> {
    const db = getAdminDb();
    const snap = await db.collection('drip_enrollments')
        .where('email', '==', email)
        .where('status', '==', 'active')
        .get();
