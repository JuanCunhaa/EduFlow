...
import { QuestionNotFoundError } from '@/lib/errors';
...
export async function importQuestions(
  ...
  for (const question of questions) {
    ...
    studyCounts.set(
      question.studyId,
      (studyCounts.get(question.studyId) || 0) + 1
    );
  }
...
export async function fetchQuestionPool(
  options: FetchQuestionPoolOptions
): Promise<Question[]> {
  const {
    uid,
    studyId,
 ...
 */
export async function isQuestionInActiveExam(
  uid: string,
  questionId: string
): Promise<boolean> {
  const db = getAdminDb();
