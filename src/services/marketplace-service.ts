...
export async function createMarketplaceQuestion(
  ...
  if (invalidDomains.length > 0) {
    throw new ValidationError(
      `Invalid domain IDs: ${invalidDomains.join(', ')}`
    );
  }
...
export async function updateMarketplaceQuestion(
  ...
  if (!existing || existing.studyId !== studyId)
    throw new MarketplaceQuestionNotFoundError();

  // If domainIds are being updated, validate against study
  if (data.domainIds) {
    const study = await adminGetDoc<MarketplaceStudy>(
      STUDIES_COL,
      existing.studyId
    );
    if (!study) throw new MarketplaceStudyNotFoundError();
    const studyDomainIds = new Set(study.domains.map((d) => d.id));
    const invalidDomains = data.domainIds.filter(
      (id) => !studyDomainIds.has(id)
    );
    if (invalidDomains.length > 0) {
      throw new ValidationError(
        `Invalid domain IDs: ${invalidDomains.join(', ')}`
...
export async function listMarketplaceStudies(
  ...
  let studies = snap.docs.map(
    (d) => ({ id: d.id, ...d.data() }) as MarketplaceStudy
  );
...
export async function getMarketplaceStudy(
  studyId: string
): Promise<MarketplaceStudy> {
  const study = await adminGetDoc<MarketplaceStudy>(STUDIES_COL, studyId);
  if (!study || !study.isActive) throw new MarketplaceStudyNotFoundError();
  return study;
}
...
export async function listMarketplaceQuestions(
  ...
  const questions = snap.docs.map(
    (d) => ({ id: d.id, ...d.data() }) as MarketplaceQuestion
  );
...
export async function importFromMarketplace(
  ...
  const mktStudy = await adminGetDoc<MarketplaceStudy>(
    STUDIES_COL,
    marketplaceStudyId
  );
  ...
  if (!existingStudiesSnap.empty) {
    ...
    const overlapping = selectedDomainIds.filter((id) =>
      alreadyImportedDomainIds.has(id)
    );
  ...
  const personalDomains = validDomains.map(
    ({ description: _, ...rest }) => rest
  );
  ...
  if (totalWrites <= FIRESTORE_BATCH_LIMIT) {
    ...
    for (const mktQuestion of allQuestions) {
      ...
      const filteredDomainIds = mktQuestion.domainIds.filter((id) =>
        selectedDomainIdSet.has(id)
      );
