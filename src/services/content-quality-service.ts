...
import { logQuestionFlagged, logQuestionArchived } from '@/services/content-audit-service';
import type {
    MarketplaceStudy,
    MarketplaceQuestion,
    Difficulty,
 ...
 */
export async function calculateCoverage(studyId: string): Promise<CertCoverage> {
    ...
    const targetPerDomain = MIN_QUESTIONS_PER_DOMAIN[certId] ?? MIN_QUESTIONS_PER_DOMAIN.default;
...
export async function recalibrateStudyDifficulty(
    ...
    const snap = await db.collection(QUESTIONS_COL)
...
export async function updateQuestionPerformanceStats(
    ...
    if (label) {
        updates[`performanceStats.optionDistribution.${label}`] = FieldValue.increment(1);
    }
...
export async function recomputeQuestionStats(
    ...
    const doc = await adminGetDoc<MarketplaceQuestion & { performanceStats?: QuestionPerformanceStats & { _totalTimeMs?: number } }>(
        QUESTIONS_COL,
        questionId
    );
    ...
    const totalTimeMs = (stats as unknown as Record<string, unknown>)._totalTimeMs as number || 0;
    ...
    await adminUpdateDoc(QUESTIONS_COL, questionId, {
        ...
        ...(calibrated ? { 'performanceStats.calibratedDifficulty': calibrated } : {}),
        updatedAt: serverTimestamp(),
    });
}
...
export async function updateQuestionLifecycle(
    ...
    if (newLifecycle === 'flagged') {
        await logQuestionFlagged(adminUid, questionId, existing.studyId, reason ?? 'Flagged for review');
    } else if (newLifecycle === 'archived') {
        await logQuestionArchived(adminUid, questionId, existing.studyId, reason ?? 'Archived');
    }
...
export async function monitorStudyQuestions(
    ...
    for (const doc of snap.docs) {
        ...
        // High skip rate
        const skipRate = stats.totalAttempts > 0
            ? stats.skipCount / stats.totalAttempts
            : 0;
...
export async function analyzeDistractors(
    ...
    for (const doc of snap.docs) {
        ...
        const mostCommonWrongPct = totalWrong > 0
            ? (maxWrongCount / totalWrong) * 100
            : 0;
