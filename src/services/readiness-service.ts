...
function computeFactors(
  ...
  const coverageValue =
    totalQuestions > 0 ? Math.min(1.0, attemptedCount / totalQuestions) : 0;
  ...
  const weakPenalty =
    domainIds.length > 0 ? 1.0 - weakDomains.length / domainIds.length : 0;
  ...
  return {
    weightedDomainAccuracy: {
      value: clamp(wda),
      weight: WEIGHTS.weightedDomainAccuracy,
    },
    recentScores: {
      value: clamp(recentValue),
      weight: WEIGHTS.recentScores,
      scores: recentScores,
    },
    hardQuestionAccuracy: {
      value: clamp(hardAcc),
      weight: WEIGHTS.hardQuestionAccuracy,
    },
    questionCoverage: {
      value: clamp(coverageValue),
      weight: WEIGHTS.questionCoverage,
      attempted: attemptedCount,
      total: totalQuestions,
    },
    weakDomainPenalty: {
      value: clamp(weakPenalty),
      weight: WEIGHTS.weakDomainPenalty,
      weakDomains,
    },
...
function computeTimeManagement(exams: Exam[]): number {
  ...
  for (const exam of exams) {
    if (!exam.config?.timeLimitMinutes || exam.config.timeLimitMinutes === 0)
      continue;
    total++;
