...
export async function createQuestionReport(
    ...
    const existingSnap = await db.collection(REPORTS_COL)
    ...
    if (data.marketplaceQuestionId) {
        try {
            const mqRef = db.collection(MARKETPLACE_QUESTIONS_COL).doc(data.marketplaceQuestionId);
            await mqRef.update({
...
export async function listQuestionReports(
    ...
    const { status, studyId, questionId, limit: limitParam = 50, cursor } = options;
    const limitCount = Math.min(Math.max(1, limitParam), 100);
    const db = getAdminDb();

    let q: FirebaseFirestore.Query = db.collection(REPORTS_COL)
    ...
    const reports = snap.docs.map(d => ({ id: d.id, ...d.data() }) as QuestionReport);
...
export async function getQuestionReport(reportId: string): Promise<QuestionReport> {
