...
function makeCollectionRef(path: string) {
  return {
    ...
    where: vi.fn(function (
      this: ReturnType<typeof makeCollectionRef>,
      _field: string,
      _op: string,
      _val: unknown
    ) {
...
function makeMarketplaceQuestion(
  id: string,
...
describe('createMarketplaceStudy', () => {
  ...
  it('does not include accentColor when omitted', async () => {
    ...
    expect(mockAdminCreateDoc.mock.calls[0][1]).not.toHaveProperty(
      'accentColor'
    );
  });
});
...
describe('updateMarketplaceStudy', () => {
  ...
  it('throws MarketplaceStudyNotFoundError when study does not exist', async () => {
    ...
    await expect(
      updateMarketplaceStudy('nonexistent', { name: 'X' })
  ...
  it('assigns domain IDs and order when updating domains', async () => {
    ...
    await updateMarketplaceStudy('mkt-study-1', {
      domains: [{ id: '', abbreviation: 'NEW', name: 'New Domain', order: 0 }],
    });
...
describe('deleteMarketplaceStudy', () => {
  ...
  it('throws MarketplaceStudyNotFoundError when study does not exist', async () => {
    ...
    await expect(deleteMarketplaceStudy('nonexistent')).rejects.toThrow(
      MarketplaceStudyNotFoundError
    );
  });
});
...
describe('createMarketplaceQuestion', () => {
  it('creates a question and updates study counters', async () => {
    ...
    const id = await createMarketplaceQuestion(
      'admin-uid',
      'mkt-study-1',
      makeQuestionInput()
    );
  ...
  it('throws when study is inactive', async () => {
    mockAdminGetDoc.mockResolvedValue(
      makeMarketplaceStudy({ isActive: false })
    );
  ...
  it('throws ValidationError for invalid domain IDs', async () => {
    ...
    await expect(
      createMarketplaceQuestion(
        'admin-uid',
        'mkt-study-1',
        makeQuestionInput({ domainIds: ['invalid'] })
      )
...
describe('bulkCreateMarketplaceQuestions', () => {
  it('creates multiple questions and returns IDs', async () => {
    ...
    const questions = [
      ...
      makeQuestionInput({
        domainIds: ['d2'],
        text: 'Question 2 about asset security',
      }),
    ];

    const result = await bulkCreateMarketplaceQuestions(
      'admin-uid',
      'mkt-study-1',
      questions
    );
  ...
  it('throws when study does not exist', async () => {
    ...
    await expect(
      bulkCreateMarketplaceQuestions('admin-uid', 'nonexistent', [
        makeQuestionInput(),
      ])
    ).rejects.toThrow(MarketplaceStudyNotFoundError);
  });
...
describe('updateMarketplaceQuestion', () => {
  it('updates a question when it exists and belongs to the study', async () => {
    ...
    await updateMarketplaceQuestion('mkt-study-1', 'q-1', {
  ...
  it('throws when question does not exist', async () => {
    ...
    await expect(
      updateMarketplaceQuestion('mkt-study-1', 'nonexistent', {
  ...
  it('throws when question belongs to a different study', async () => {
    ...
    await expect(
      updateMarketplaceQuestion('mkt-study-1', 'q-1', {
        text: 'Updated text value',
      })
    ).rejects.toThrow(MarketplaceQuestionNotFoundError);
  });
...
describe('getMarketplaceStudy', () => {
  ...
  it('throws when study does not exist', async () => {
    ...
    await expect(getMarketplaceStudy('nonexistent')).rejects.toThrow(
      MarketplaceStudyNotFoundError
    );
  });
  ...
  it('throws when study is inactive', async () => {
    ...
    await expect(getMarketplaceStudy('mkt-study-1')).rejects.toThrow(
      MarketplaceStudyNotFoundError
    );
  });
});
...
describe('importFromMarketplace', () => {
  ...
  it('imports study and questions to user namespace', async () => {
    ...
    const result = await importFromMarketplace(uid, marketplaceStudyId, [
      'd1',
      'd2',
    ]);
  ...
  it('allows importing different domains from the same study', async () => {
    setupImportScenario({
      ...
      questions: [
        {
          id: 'q3',
          data: makeMarketplaceQuestion('q3', { domainIds: ['d2'] }),
        },
      ],
    });
  ...
  it('uses batch write for atomic import', async () => {
    setupImportScenario({
      questions: [
        {
          id: 'q1',
          data: makeMarketplaceQuestion('q1', { domainIds: ['d1'] }),
        },
      ],
    });
  ...
  it('filters question domainIds to only selected domains', async () => {
    setupImportScenario({
      questions: [
        {
          id: 'q1',
          data: makeMarketplaceQuestion('q1', {
            domainIds: ['d1', 'd2', 'd3'],
          }),
        },
      ],
  ...
  it('returns correct result shape', async () => {
    setupImportScenario({
      questions: [
        ...
        {
          id: 'q2',
          data: makeMarketplaceQuestion('q2', { domainIds: ['d2'] }),
        },
      ],
    });
