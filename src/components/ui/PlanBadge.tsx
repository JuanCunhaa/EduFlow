...
export function PlanBadge({ compact = false, className }: PlanBadgeProps) {
  ...
  const { plan, isPro, isTrialing, isCanceling, isPastDue, periodEnd } =
    usePlan();
  ...
  if (plan === 'free') {
    return (
      <Link
        href="/pricing"
        className="group hover:bg-accent/30 flex items-center gap-2 rounded-lg px-1 py-0.5 transition-all duration-200"
      >
        {badge}
        {!compact && (
          <span className="text-primary text-xs font-medium opacity-80 transition-opacity group-hover:opacity-100">
            {t('cta.upgrade')}
          </span>
        )}
      </Link>
