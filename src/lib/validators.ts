...
export const explanationSchema = z.object({
  short: safeString(10),
  whyOthersWrong: z
    .record(z.string().max(1), z.string().transform(stripHtml))
    .default({}),
});
...
export const examConfigSchema = z.object({
  ...
  questionCount: z
    .number()
    .int()
    .refine((n: number) => [10, 25, 50, 100, 150].includes(n), {
...
export const createMarketplaceStudySchema = z.object({
  ...
  accentColor: z
    ...
    .optional(),
  tags: z
    .array(z.string().transform(stripHtml).pipe(z.string().max(50)))
    .max(20)
    .default([]),
});

export const updateMarketplaceStudySchema =
  createMarketplaceStudySchema.partial();
...
export const createMarketplaceQuestionSchema =
  marketplaceQuestionBaseSchema.refine(
...
export const updateMarketplaceQuestionSchema =
  marketplaceQuestionBaseSchema.partial();
...
export const questionLifecycleSchema = z.enum([
  'active',
  'flagged',
  'archived',
  'revised',
...
export const contentBatchQuestionSchema = z
  .object({
    text: safeString(20),
    options: z.array(optionSchema).min(4).max(5),
    correctOptionIndex: z.number().int().min(0).max(4),
...
export type CreateMarketplaceStudyInput = z.infer<
  typeof createMarketplaceStudySchema
>;
export type UpdateMarketplaceStudyInput = z.infer<
  typeof updateMarketplaceStudySchema
>;
export type CreateMarketplaceQuestionInput = z.infer<
  typeof createMarketplaceQuestionSchema
>;
export type UpdateMarketplaceQuestionInput = z.infer<
  typeof updateMarketplaceQuestionSchema
>;
export type MarketplaceBulkQuestionsInput = z.infer<
  typeof marketplaceBulkQuestionsSchema
>;
export type MarketplaceImportInput = z.infer<typeof marketplaceImportSchema>;
export type CreateQuestionReportInput = z.infer<
  typeof createQuestionReportSchema
>;
export type ResolveQuestionReportInput = z.infer<
  typeof resolveQuestionReportSchema
>;
export type CreateContentAuditInput = z.infer<typeof createContentAuditSchema>;
export type UpdateQuestionLifecycleInput = z.infer<
  typeof updateQuestionLifecycleSchema
>;
export type ContentBatchInput = z.infer<typeof contentBatchSchema>;
export type ContentBatchQuestionInput = z.infer<
  typeof contentBatchQuestionSchema
>;
