...
describe('withAuth', () => {
    ...
    it('returns 413 for oversized request body', async () => {
        const handler = withAuth(async () => ({ data: 'ok' }), { maxBodySize: 100 });
        const req = new Request('http://localhost/api/test', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
    ...
    it('allows request within body size limit', async () => {
        const handler = withAuth(async () => ({ data: 'ok' }), { maxBodySize: 1000 });
        const req = new Request('http://localhost/api/test', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
    ...
    it('uses verifyAuthStrict when checkRevoked=true', async () => {
        const handler = withAuth(async (_req, { user }) => ({ data: user.uid }), { checkRevoked: true });
...
describe('withPublicRoute', () => {
    ...
    it('resolves async route params', async () => {
        const handler = withPublicRoute(async (_req, { params }) => ({ data: params }));
