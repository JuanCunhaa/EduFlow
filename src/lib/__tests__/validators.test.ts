...
describe('createQuestionSchema', () => {
  ...
  it('accepts question with 5 options', () => {
    const fiveOpts = {
      ...validQuestion,
      options: [...validQuestion.options, { label: 'E', text: 'Extra option' }],
    };
...
describe('examConfigSchema', () => {
  ...
  it.each([10, 25, 50, 100, 150])(
    'accepts valid questionCount: %d',
    (count: number) => {
      expect(() =>
  ...
  it.each([0, 5, 15, 30, 75, 200])(
    'rejects invalid questionCount: %d',
    (count: number) => {
      expect(() =>
        examConfigSchema.parse({ ...validConfig, questionCount: count })
      ).toThrow();
...
describe('studyDomainSchema', () => {
  ...
  it('rejects id longer than 20 chars', () => {
    expect(() =>
      studyDomainSchema.parse({ ...validDomain, id: 'x'.repeat(21) })
    ).toThrow();
  });

  it('rejects negative order', () => {
    expect(() =>
      studyDomainSchema.parse({ ...validDomain, order: -1 })
    ).toThrow();
  });
  ...
  it('strips HTML from name', () => {
    const result = studyDomainSchema.parse({
      ...validDomain,
      name: '<b>Security</b>',
    });
