...
function selectWeakDomains(
  ...
  if (Object.keys(domainAccuracy).length === 0)
    return roundRobinSelect(pool, count);
  ...
  const weakCount = Math.min(
    Math.ceil(count * WEAK_DOMAIN_RATIO),
    scoredWeak.length
  );
...
function computeWeakScore(
  ...
  if (!attempt) {
    ...
    const missRate =
      attempt.attempts > 0 ? 1 - attempt.correct / attempt.attempts : 0;
...
function selectRecentMisses(
  ...
  const selected: Question[] = weightedSample(
    missed,
    Math.min(count, missed.length)
  );
...
function selectSpacedReview(
  ...
  if (selected.length < count) {
    ...
    selected.push(
      ...upcoming.slice(0, count - selected.length).map((s) => s.question)
    );
  }
