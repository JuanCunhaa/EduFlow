...
function detectAnomalies(request: Request): { score: number; reasons: string[] } {
    let score = 0;
    ...
    if (!ua || ua.length < 10) {
        ...
        const botPatterns = /bot|crawler|spider|scraper|curl|wget|httpie|python|node-fetch|axios|postman/i;
...
export async function checkScrapingSignals(
    ...
    const minuteAllowed = await rateLimit(minuteKey, maxRequestsPerMinute, 60_000, true);
    if (!minuteAllowed) {
        score += 30;
    ...
    const hourAllowed = await rateLimit(hourKey, maxRequestsPerHour, 3_600_000, true);
    if (!hourAllowed) {
        score += 25;
