...
import type { PlanTier, StripeSubStatus, PaywallFeature, UserProfile } from '@/types';
 ...
 */
export function resolveEffectivePlan(profile: UserProfile | null, isAdmin = false): PlanTier {
    ...
    if (plan === 'pro') {
        ...
        if (status === 'canceled' && planPeriodEnd && planPeriodEnd > Date.now()) return 'pro';

        // Canceled and past period end → Free
        if (status === 'canceled' && planPeriodEnd && planPeriodEnd <= Date.now()) return 'free';
 ...
 */
export function meetsRequirement(current: PlanTier, required: PlanTier): boolean {
...
async function checkMarketplaceImportLimit(uid: string): Promise<PlanLimitCheck> {
    const db = getAdminDb();
...
async function checkQuestionCreationLimit(uid: string): Promise<PlanLimitCheck> {
    const db = getAdminDb();
...
export function getMaxQuestionsPerExam(plan: PlanTier): number {
    return plan === 'free' ? FREE_MAX_QUESTIONS_PER_EXAM : PRO_MAX_QUESTIONS_PER_EXAM;
}
