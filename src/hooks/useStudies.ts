...
export function useStudies() {
    const { data, error, isLoading, mutate: refresh } = useSWR<Study[]>(
        '/api/studies',
        fetcher,
...
function invalidateStudies() {
    mutate((key: string) => typeof key === 'string' && key.startsWith('/api/studies'));
}
...
export async function updateStudy(studyId: string, data: UpdateStudyInput): Promise<void> {
...
export async function deleteStudy(studyId: string): Promise<{ deletedQuestions: number; deletedExams: number }> {
    const res = await fetch(`/api/studies/${studyId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    });
