...
interface UsePaywallReturn {
    ...
    showPaywall: (feature: PaywallFeature, currentUsage?: number, limit?: number) => void;
...
export function usePaywall(): UsePaywallReturn {
    ...
    const handlePaywallError = useCallback((error: unknown): boolean => {
        ...
        if (error && typeof error === 'object' && 'code' in error) {
            const apiError = error as { code?: string; details?: Record<string, unknown> };
    ...
    const showPaywall = useCallback((feature: PaywallFeature, currentUsage?: number, limit?: number) => {
