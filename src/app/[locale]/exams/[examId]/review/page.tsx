      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="border-border text-muted-foreground hover:text-foreground inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm transition-colors"
          >
          ...
          {data.questions.map((q, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`mt-0.5 flex-none rounded-full p-1 ${q.isCorrect ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}
                  >
                  ...
                  {revealed ? (
                    <ChevronUp
                      size={16}
                      className="text-muted-foreground mt-1 flex-none"
                    />
                  ) : (
                    <ChevronDown
                      size={16}
                ...
                {revealed && (
                  <div className="border-border space-y-4 border-t px-4 pt-3 pb-4">
                    ...
                    <ul className="space-y-2">
                      {q.options.map((opt, i) => {
                        ...
                        return (
                          <li
                            key={i}
                            className={`rounded-lg px-3 py-2 text-sm ${style}`}
                          >
                    ...
                    {/* Explanation */}
                    {q.explanation.short && (
                      <div className="bg-muted/30 space-y-1 rounded-lg px-3 py-2">
                    ...
                    {Object.keys(q.explanation.whyOthersWrong).length > 0 && (
                      <div className="bg-muted/30 space-y-1 rounded-lg px-3 py-2">
                        ...
                        <ul className="space-y-1">
                          {Object.entries(q.explanation.whyOthersWrong).map(
                            ([label, reason]) => (
                              <li
                                key={label}
                    ...
                    <div className="space-y-2">
                      <button
                        onClick={() =>
                          setOpenNoteId(openNoteId === q.id ? null : q.id)
                        }
                      ...
                      {openNoteId === q.id && (
                        <div className="flex gap-2">
                          <textarea
                            ...
                            className="border-border bg-background text-foreground placeholder:text-muted-foreground flex-1 resize-none rounded-md border px-3 py-2 text-sm"
                            rows={2}
                          />
                          <button
                            onClick={() => saveNote(q.id)}
                            disabled={savingNoteId === q.id}
                            className="bg-primary text-primary-foreground hover:bg-primary/90 self-end rounded-md px-3 py-1.5 text-xs font-medium disabled:opacity-50"
                          >
