    ...
    const load = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const published = (data.packs || []).filter(
            (p: QuestionPack) => p.status === 'published'
          );
          setStats({
            totalPacks: data.packs?.length || 0,
            published: published.length,
            totalSales: published.reduce(
              (sum: number, p: QuestionPack) => sum + p.salesCount,
              0
            ),
    ...
    <Shell>
      <div className="mx-auto max-w-4xl py-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <Link
            href="/marketplace/creators/apply"
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div
              key={label}
              className="bg-surface border-border rounded-lg border p-4 text-center"
            >
        ...
        {/* Packs List */}
        <h2 className="mb-4 text-lg font-semibold">Your Packs</h2>
        {packs.length === 0 ? (
          <div className="bg-surface border-border rounded-lg border py-12 text-center">
          ...
          <div className="space-y-3">
            {packs.map((pack) => {
              ...
              return (
                <div
                  key={pack.id}
                  className="bg-surface border-border hover:border-primary flex items-center gap-4 rounded-lg border p-4 transition-colors"
                >
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate font-semibold">{pack.title}</h3>
                    <div className="text-muted mt-1 flex items-center gap-3 text-sm">
                  ...
                  <div
                    className={`flex items-center gap-1.5 text-sm ${cfg.color}`}
                  >
