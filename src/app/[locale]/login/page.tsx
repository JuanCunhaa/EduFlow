        ...
        <div className="border-border glass-panel w-full rounded-2xl border p-8">
          <button
            onClick={async () => {
              ...
              try { await signIn(); } catch (err) {
                ...
                if (code !== 'auth/popup-closed-by-user' && code !== 'auth/cancelled-popup-request') setError(t('error'));
          ...
          <button
            ...
            onClick={async () => {
              ...
              setSigningIn(true);
              setError(null);
              try { await signInGitHub(); } catch (err) {
                const code = (err as { code?: string })?.code;
                if (code !== 'auth/popup-closed-by-user' && code !== 'auth/cancelled-popup-request') setError(t('error'));
              } finally { setSigningIn(false); }
            }}
            disabled={signingIn}
            className="border-border text-foreground hover:bg-accent flex w-full items-center justify-center gap-3 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-60"
          >
            {/* GitHub mark */}
          ...
          {error && (
            <p className="animate-fade-in text-center text-sm text-red-400">{error}</p>
          )}
        </div>
