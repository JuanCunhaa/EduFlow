                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                            ...
                            {comp.products.map((p) => (
                                <th key={p.name} style={{ textAlign: 'center', padding: '0.75rem' }}>{p.name}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {comp.features.map((feature, i) => (
                            <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                <td style={{ padding: '0.75rem', fontWeight: 500 }}>{feature.name}</td>
                                {feature.values.map((val, j) => (
                                    <td key={j} style={{ textAlign: 'center', padding: '0.75rem' }}>
            ...
            {comp.sections.map((section, i) => (
                <section key={i} className="seo-card" style={{ marginTop: '1.5rem' }}>
                    ...
                    {section.paragraphs.map((p, j) => (
                        <p key={j} style={{ lineHeight: 1.7, marginBottom: '1rem' }}>{p}</p>
                    ))}
                </section>
            ))}
