  ...
  return (
    <>
      ...
      <section className="seo-hero">
        ...
        <p>
          Everything you need to know about the {cert.abbreviation} exam
          structure, format, and what to expect on test day.
        </p>
      </section>
      ...
      <section className="seo-card">
        ...
        <dl className="seo-exam-grid">
          <div>
            <dt>Duration</dt>
            <dd>{cert.examDetails.duration}</dd>
          </div>
          <div>
            <dt>Questions</dt>
            <dd>{cert.examDetails.questions}</dd>
          </div>
          <div>
            <dt>Format</dt>
            <dd>{cert.examDetails.format}</dd>
          </div>
          <div>
            <dt>Passing Score</dt>
            <dd>{cert.examDetails.passingScore}</dd>
          </div>
          <div>
            <dt>Cost</dt>
            <dd>{cert.examDetails.cost}</dd>
          </div>
          <div>
            <dt>Prerequisites</dt>
            <dd>{cert.examDetails.prerequisites}</dd>
          </div>
        </dl>
      </section>
          ...
          {isCAT ? (
            <>
              <p style={{ lineHeight: 1.7 }}>
                The {cert.abbreviation} exam uses{' '}
                <strong>Computerized Adaptive Testing (CAT)</strong>. Unlike
                traditional linear exams, CAT dynamically adjusts the difficulty
                of questions based on your performance throughout the exam.
              </p>
              <p style={{ lineHeight: 1.7 }}>
                When you answer a question correctly, the next question will be
                harder. When you answer incorrectly, the next question will be
                easier. The exam engine uses this pattern to efficiently
                determine your true competency level with fewer questions.
              </p>
              <p style={{ lineHeight: 1.7 }}>
                You may be asked between {cert.examDetails.questions} questions.
                The exam ends when the engine has enough confidence in your
                pass/fail determination, or when you reach the maximum question
                count.
              </p>
            </>
          ) : (
            <>
              <p style={{ lineHeight: 1.7 }}>
                The {cert.abbreviation} exam uses a{' '}
                <strong>linear format</strong>. All candidates receive the same
                number of questions ({cert.examDetails.questions}) in a fixed
                sequence. This means you can review and change your answers
                before submitting.
              </p>
              <p style={{ lineHeight: 1.7 }}>
                Time management is critical — you have{' '}
                {cert.examDetails.duration} to answer all{' '}
                {cert.examDetails.questions} questions, so pace yourself
                accordingly.
              </p>
            </>
          )}
      ...
      <section className="seo-section">
        ...
        <div className="seo-domain-list">
          {cert.domains.map((domain) => (
            ...
            >
              <span className="seo-domain-item__number">
                D{domain.domainNumber}
              </span>
              <span className="seo-domain-item__name">{domain.name}</span>
              <span className="seo-domain-item__weight">
                {domain.examWeight}
              </span>
            </Link>
          ))}
      ...
      <section className="seo-section">
        ...
        >
          ...
          <div className="seo-card">
            ...
            <p style={{ margin: 0 }}>
              Read each question stem carefully. Eliminate obviously wrong
              answers first, then choose the BEST answer — not just a correct
              one.
            </p>
          </div>
          <div className="seo-card">
