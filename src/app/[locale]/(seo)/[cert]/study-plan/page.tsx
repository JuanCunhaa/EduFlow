    ...
    return (
        <>
            ...
            <section className="seo-hero">
                ...
                <p>
                    A structured {plan.totalWeeks}-week study plan to pass the {cert.abbreviation}{' '}
                    certification. Includes weekly milestones, domain priorities, and readiness checkpoints.
                </p>
            </section>
            ...
            <section className="seo-card">
                ...
                <dl className="seo-exam-grid">
                    <div><dt>Duration</dt><dd>{plan.totalWeeks} weeks</dd></div>
                    <div><dt>Hours/Week</dt><dd>{plan.hoursPerWeek}</dd></div>
                    <div><dt>Total Hours</dt><dd>{plan.totalWeeks * plan.hoursPerWeek}</dd></div>
                    <div><dt>Domains</dt><dd>{cert.domains.length}</dd></div>
                </dl>
            </section>
                ...
                <div className="seo-domain-list">
                    {plan.weeks.map((week, i) => (
                        <div key={i} className="seo-card" style={{ padding: '1.5rem' }}>
                            ...
                            <p style={{ margin: '0 0 0.75rem', opacity: 0.7 }}>{week.focus}</p>
                            <ul style={{ margin: 0, paddingLeft: '1.25rem' }}>
                                {week.tasks.map((task, j) => (
                                    <li key={j} style={{ marginBottom: '0.25rem' }}>{task}</li>
                                ))}
                            </ul>
                        </div>
                    ))}
