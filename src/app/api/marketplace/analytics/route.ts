...
export const GET = withAuth(async (req, { user }) => {
  ...
  const packs = studiesSnap.docs.map((doc) => {
    const data = doc.data();
    const packPurchases = purchasesSnap.docs.filter(
      (p) => p.data().studyId === doc.id
    );
    const packPeriodPurchases = packPurchases.filter(
      (p) => p.data().createdAt >= cutoff
    );
    ...
    return {
      ...
      totalRevenue: packPurchases.reduce(
        (sum, p) => sum + (p.data().price || 0),
        0
      ),
      periodDownloads: packPeriodPurchases.length,
      periodRevenue: packPeriodPurchases.reduce(
        (sum, p) => sum + (p.data().price || 0),
        0
      ),
    };
  ...
  const avgRating =
    ratedPacks.length > 0
      ? Math.round(
          (ratedPacks.reduce((sum, p) => sum + p.rating, 0) /
            ratedPacks.length) *
            10
        ) / 10
      : 0;
  ...
  return {
    ...
    totalDownloads: studiesSnap.docs.reduce(
      (sum, d) => sum + (d.data().downloadCount || 0),
      0
    ),
    totalRevenue,
    avgRating,
