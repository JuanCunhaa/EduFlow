...
export const GET = withAdmin(async (req) => {
    ...
    const snap = await db.collection('marketplace_studies')
        .where('status', '==', status)
...
export const PUT = withAdmin(async (req, { user }) => {
    ...
    if (!validActions.includes(action)) {
        throw Object.assign(new Error(`action must be one of: ${validActions.join(', ')}`), { status: 400 });
    }

    if (action !== 'approve' && (!reason || typeof reason !== 'string')) {
        throw Object.assign(new Error('reason required for reject/request_changes'), { status: 400 });
    }
