...
export const POST = withAuth(async (req, { user }) => {
    ...
    if (!certId || typeof passed !== 'boolean') {
        throw Object.assign(new Error('certId (string) and passed (boolean) required'), { status: 400 });
    }
...
export const GET = withAuth(async (req) => {
    ...
    if (!certId) {
        throw Object.assign(new Error('certId query param required'), { status: 400 });
    }
    ...
    return {
        ...
        avgReadinessAtExam: readinessCount > 0 ? Math.round(totalReadiness / readinessCount) : null,
    };
});
