...
export const GET = withAuth(async (_request, { user, log }) => {
    ...
    const orgIds = memberSnaps.docs.map((doc) => doc.ref.parent.parent?.id).filter(Boolean) as string[];
...
export const POST = withAuth(async (request, { user, log }) => {
    ...
    const existing = await db.collection('orgs').where('slug', '==', slug).limit(1).get();
    if (!existing.empty) {
