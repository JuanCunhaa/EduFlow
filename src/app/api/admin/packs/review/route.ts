...
export const PATCH = withAdmin(async (request, { user, log }) => {
    ...
    if (!packId || !decision) {
        return NextResponse.json({ error: 'packId and decision required' }, { status: 400 });
    }
    ...
    if (!['submitted', 'in_review'].includes(pack.status)) {
        return NextResponse.json({ error: `Pack is not in review queue (status: ${pack.status})` }, { status: 400 });
    }
    ...
    if (decision === 'approved') {
        ...
        await db.collection('creators').doc(pack.creatorId).update({
