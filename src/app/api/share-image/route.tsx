...
export async function GET(request: Request) {
  ...
  const goalPercent = Math.min(
    100,
    Math.round((todayAnswered / stats.dailyGoal) * 100)
  );

  // Calculate overall accuracy from recent 30 days
  const totalCorrect = stats.recentDays.reduce(
    (sum, d) => sum + d.correctAnswers,
    0
  );
  const totalAnswered = stats.recentDays.reduce(
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        <div style={{ fontSize: '14px', color: '#64748b' }}>{displayName}</div>
        <div style={{ fontSize: '14px', color: '#64748b' }}>
