...
export const POST = withAuth(async (request, { user, log }) => {
    ...
    if (!creatorDoc.exists || creatorDoc.data()?.verificationStatus !== 'approved') {
        return NextResponse.json({ error: 'Creator account required' }, { status: 403 });
    }

    const body = await request.json();
    const { certId, title, description, domains, pricing, priceUsd, tags, accentColor } = body;

    if (!certId || !title || !description) {
        return NextResponse.json({ error: 'certId, title, and description are required' }, { status: 400 });
    }
    if (title.length > 80) {
        return NextResponse.json({ error: 'Title must be ≤ 80 characters' }, { status: 400 });
    }

    const creator = creatorDoc.data()!;
    const slug = `${certId}-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`;
