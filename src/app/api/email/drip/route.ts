...
import { enrollInSequence, processDueEnrollments } from '@/services/email-drip-service';
...
export const POST = withAuth(async (req, { user }) => {
    ...
    const validSequences: DripSequenceId[] = ['welcome', 're_engagement', 'cert_tips', 'upgrade_nudge'];
    if (!validSequences.includes(sequenceId)) {
        throw Object.assign(new Error(`sequenceId must be one of: ${validSequences.join(', ')}`), { status: 400 });
    }
    ...
    const enrollmentId = await enrollInSequence(enrollmentEmail, user.uid, sequenceId);
    return { enrollmentId };
});
