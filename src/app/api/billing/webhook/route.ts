...
export async function POST(request: Request) {
    ...
    try {
        ...
        log.error('Error processing webhook event', { error: err, meta: { eventId: event.id, type: event.type } });
...
async function handleCheckoutCompleted(
    ...
    if (!userSnap.exists) {
        log.warn('User not found for checkout completion', { meta: { uid: firebaseUid } });
        return;
    }

    const isTrialing = subscription.status === 'trialing';
    const periodEnd = (subscription.items.data[0]?.current_period_end ?? 0) * 1000;
...
async function handleSubscriptionUpdated(
    ...
    log.info('Subscription updated', {
        meta: { uid, status: subscription.status, cancelAtPeriodEnd: subscription.cancel_at_period_end },
    });
}
...
async function handleInvoicePaymentFailed(
    ...
    if (!uid) {
        log.warn('User not found for invoice.payment_failed', { meta: { customerId } });
        return;
    }
