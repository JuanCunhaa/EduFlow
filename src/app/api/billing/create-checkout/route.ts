...
export const POST = withAuth(async (request: Request, { user, log }: RouteContext) => {
    ...
    const { priceId, successUrl = '/dashboard?checkout=success', cancelUrl = '/pricing' } = body;
    ...
    if (profile?.stripeSubscriptionId && profile?.stripeSubscriptionStatus) {
        ...
        if (activeStatuses.includes(profile.stripeSubscriptionStatus)) {
            throw new ConflictError('You already have an active subscription. Manage it from your billing settings.');
        }
    }
    ...
    log.info('Checkout session created', { meta: { sessionId: session.id, priceId } });
