...
export async function GET(request: Request) {
    ...
    let checked = 0;
    let errors = 0;
    const discrepancies: Array<{ uid: string; field: string; firestore: unknown; stripe: unknown }> = [];
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        try {
            ...
            // Check period end
            const stripePeriodEnd = (subscription.items.data[0]?.current_period_end ?? 0) * 1000;
            ...
            const shouldBePro = activeStatuses.includes(subscription.status) ||
            ...
            if (error.statusCode === 404) {
                ...
                errors++;
                log.error('Reconciliation error for user', { error: err, meta: { uid: userDoc.id } });
            }
        }
    }
