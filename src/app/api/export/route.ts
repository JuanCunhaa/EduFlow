  ...
  const rows = exams.map((exam) => {
    const date = exam.completedAt
      ? typeof exam.completedAt === 'object' && 'seconds' in exam.completedAt
        ? new Date((exam.completedAt as { seconds: number }).seconds * 1000)
            .toISOString()
            .split('T')[0]
        : new Date(exam.completedAt as unknown as string)
            .toISOString()
            .split('T')[0]
      : '';
    const studyName = studyMap.get(exam.studyId) || exam.studyId;
    const questionCount =
      exam.config?.questionCount || exam.questionIds?.length || 0;
    const correct =
      exam.score != null ? Math.round((exam.score / 100) * questionCount) : 0;
    const timeMin = exam.timeSpentSeconds
      ? Math.round(exam.timeSpentSeconds / 60)
      : 0;
