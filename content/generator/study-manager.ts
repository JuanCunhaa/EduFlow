...
export class StudyManager {
  ...
  async findOrCreateStudy(meta: StudyMetadata): Promise<string> {
    ...
    if (meta.studyId) {
      ...
      if (doc.exists) {
        ...
        if (!data?.tags || data.tags.length === 0) {
          ...
          console.log(
            `   ✨ Backfilled missing tags for study: ${meta.studyId}`
          );
        }
        return meta.studyId;
      }
      console.warn(
        `   ⚠️  Study ID ${meta.studyId} from file not found in Firestore. Creating new one...`
      );
    }
    ...
    if (!snapshot.empty) {
      ...
      console.log(
        `   ✅ Found existing study for ${meta.certSlug}: ${existingId}`
      );
      return existingId;
    }
