...
import { validateBatch, type GeneratedQuestion } from './question-validator';
...
async function main() {
  ...
  console.log(
    `🎯 Generating ${args.count} per domain for ${domainsToGenerate.length} domains`
  );
  ...
  if (!studyId && cert.slug) {
    ...
    if (fs.existsSync(defaultPath)) {
      try {
        ...
        if (saved.studyId) {
          console.log(
            `   📂 Found existing study file: ${path.relative(process.cwd(), defaultPath)}`
          );
  ...
  if (args.dryRun) {
    ...
    try {
      ...
      console.log(
        `   💾 Updated study metadata: ${path.relative(process.cwd(), studyFilePath)}`
      );
    } catch (err) {
  ...
  for (const domain of domainsToGenerate) {
    if (args.dryRun) {
      console.log(`\n${'═'.repeat(60)}`);
      console.log(
        `📚 Domain ${domain.domainNumber}: ${domain.domainName} (${domain.domainWeight}%)`
      );
    ...
    while (generatedForDomain < totalNeeded) {
      ...
      try {
        ...
        const validQuestions = questions.filter(
          (_, i) => validation.results[i].valid
        );
        const invalidCount = questions.length - validQuestions.length;
        console.log(
          `   📊 Difficulty: easy=${validation.difficultyDistribution.easy} medium=${validation.difficultyDistribution.medium} hard=${validation.difficultyDistribution.hard}`
        );

        if (validQuestions.length > 0) {
          const batchPath = saveBatch(
            cert,
            domain,
            validQuestions,
            args.model,
          ...
          console.log(
            `   💾 Saved batch: ${path.relative(process.cwd(), batchPath)}`
          );
        }
  ...
  if (!args.noImport && !args.dryRun && batchFiles.length > 0) {
    ...
    if (!args.autoApprove) {
      ...
      const answer = await new Promise<string>((resolve) => {
        rl.question(
          '\n   👉 Proceed to import to Marketplace? (Y/n) ',
          resolve
        );
      });
      rl.close();
