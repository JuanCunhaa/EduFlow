...
export function validateQuestion(q: GeneratedQuestion, index: number): ValidationResult {
    ...
    if (q.text.length < MIN_STEM_LENGTH) {
        errors.push(`${prefix}: Stem too short (${q.text.length} chars, min ${MIN_STEM_LENGTH})`);
    }

    if (!Array.isArray(q.options) || q.options.length !== 4) {
        errors.push(`${prefix}: Must have exactly 4 options, got ${q.options?.length ?? 0}`);
    }

    if (typeof q.correctOptionIndex !== 'number' || q.correctOptionIndex < 0 || q.correctOptionIndex > 3) {
        errors.push(`${prefix}: correctOptionIndex must be 0–3, got ${q.correctOptionIndex}`);
    }

    if (!VALID_DIFFICULTIES.has(q.difficulty)) {
        errors.push(`${prefix}: Invalid difficulty "${q.difficulty}". Must be easy/medium/hard`);
    }
    ...
    if (Array.isArray(q.options) && q.options.length === 4) {
        ...
        if (q.options.length === 4) {
            ...
            if (lengths[q.correctOptionIndex] === maxLen &&
        ...
        for (const opt of q.options) {
            ...
            if (lower.includes('all of the above') || lower.includes('none of the above')) {
                errors.push(`${prefix}: Contains "all/none of the above" — not allowed`);
            }
        }
    }
    ...
    if (!q.explanation?.short) {
        ...
        if (sentences.length < MIN_EXPLANATION_SENTENCES) {
            errors.push(`${prefix}: Explanation needs ${MIN_EXPLANATION_SENTENCES}+ sentences, got ${sentences.length}`);
        }
    }
    ...
    for (const fake of FAKE_NIST_NUMBERS) {
        if (fullText.includes(`SP ${fake}`) || fullText.includes(`800-${fake.replace('800-', '')}`)) {
            warnings.push(`${prefix}: Possibly fabricated NIST SP ${fake} — verify manually`);
        }
    }
