...
export class OpenAIClient {
    ...
    constructor(config: OpenAIConfig) {
        ...
        const isReasoning = (config.model || 'gpt-4o').startsWith('o1-') || (config.model || 'gpt-4o').startsWith('o3-') || (config.model || 'gpt-4o').startsWith('gpt-5-');
    ...
    async chat(messages: OpenAIMessage[]): Promise<OpenAIResponse> {
        return this.withRetry(async () => {
            ...
            const isReasoningModel = this.config.model.startsWith('o1-') || this.config.model.startsWith('o3-') || this.config.model.startsWith('gpt-5-');
            ...
            if (isReasoningModel) {
                console.log(`   🧠 Reasoning model (${this.config.model}) is thinking... this may take 1-2 minutes.`);
            }
            ...
            if (!response.ok) {
                const errorBody = await response.json().catch(() => ({})) as OpenAIError;
     ...
     */
    async chatJSON<T = unknown>(messages: OpenAIMessage[]): Promise<{
        data: T;
        usage: OpenAIUsage;
        model: string;
    }> {
