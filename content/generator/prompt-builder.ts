...
export function getDomainMeta(
  ...
  if (!domain) {
    const available = cert.domains
      .map((d) => `${d.id} (${d.number}: ${d.name})`)
      .join(', ');
    throw new Error(
      `Unknown domain "${domainIdOrNumber}" for ${cert.name}. Available: ${available}`
    );
  }
...
export function buildSystemPrompt(
  ...
  const isSecurityCert = securityIssuers.some((s) =>
    domain.certIssuer.includes(s)
  );
...
async function discoverCertDomains(
  certInput: string,
  client: AIClient
): Promise<CertInfo> {
...
Output ONLY the JSON. No markdown, no explanation.`;
  ...
  if (
    ...
    throw new Error(
      'AI returned invalid cert structure. Try again or use a more specific cert name.'
    );
  }
  ...
  for (const d of cert.domains) {
    console.log(
      `      ${d.number}. ${d.name} (${d.weight}) — ${d.topics.length} topics`
    );
  }
